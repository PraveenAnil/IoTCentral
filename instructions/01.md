...
1. In the edit dashboard page in the left navigation bar under **Dashboard settings**,  enter a friendly name for your dashboard in **Dashboard name**. Optionally you can also select the type of visibility you want.
...
1. In the application page from the left navigation menu  select **Device templates** under **App settings**. Templates contain device capabilities. They also determine how you view data from your devices.
...
1. In the application page from the left navigation pane, select **Devices** this list all the devices that are connected to your application.
...
1. In create a new device page enter the following details:
     ...
     - **Device ID**: Enter the id of your device
 ...
 1. In the **Device Dashboard** you can see status of your device, transportation mode, Battery life, Location of the device and tags.
 
 1. Navtigate to **Commands** tab from here you can run **Gateway Command**. Under **Gateway Commands / Disable Sensor** click on **Run**. 
...
1. On this view, you can select the columns to display and set a time range to view. 
...
1. Under **Target devices** section select the target device group that you want your job to apply to. For now select **Logistics Gateway - All devices**.

1. In **Job properties** section under **Job type** select **Command** from the drop down.

1. Select **Update Firmware** under command and click on **Next** to go to  **Delivery Options** page.
...
1. In the **Device file upload** tab enter the following details:
     ...
     - **Storage account**: Select storage account with name **iotcentralfileuploadxxx**.
...
1. In the Analysis page click on **Change Y-axis type** from here you can chnage the Y-axis format. You can set the y-axis by choosing from one of three modes:
...
1. From the dropdown, select **View data as Table** from here you can see results in table format, this enables you to view the specific value for each data point.

1. In this way you can analyse diffrent telemetry options available.

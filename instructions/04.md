...
### Task 2: Integrate IoT Central Application with other cloud services or existing enterprise and line of business solutions 
  ...
  In this task, you will configure a Logic App to be triggered by a custom rule in which if the average temperature of the device sensor exceeds a certain value, then the Logic App executes a gateway command that disables and enables the device.
...
1. In the **Execute a device command(preview)** page, select the following:
 ...
 1. To verify the trigger has been fired, switch to **Azure portal** and select your logic app. Under **Evalutaion**  click on **See trigger history** you should be able to see actions succeeded.
 ...
 In this task, you will configure your IoT Central application to export telemetry collected from the devices, to Azure storage account.
